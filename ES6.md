...
> ES6对基于类面向过程方式的补充，和其他语言相比，还是有所不如，只是实现了最基本的三大特性，至于更多的一些还有待补充：比如，访问权限[私有属性、私有方法等等] ?
...
优点: 完全能够预测代码的执行过程，书写过程即执行过程。

缺点: 同步执行遇到耗时操作，容易阻塞整体执行过程，影响体验。

##### 异步编程的概念：调整代码执行顺序，将耗时操作延后。
调整JS代码的执行顺序，让JS代码的执行顺序 [异于] 代码的书写顺序。个人感觉异步里的异一词的含义时区别于正常的代码执行顺序，而不是在另一个线程执行的意思。

优点: 能将耗时操作延迟执行，避免线程阻塞。

缺点: 代码的执行顺序将变得不可控 [指开发者理解上的代码执行顺序不可控，不是实际代码执行顺序，实际代码执行每一次的顺序肯定是不变的] ，尤其碰到多开线程，就更复杂了。

##### 异步编程的解决方案: 多开线程
> js属于单线程执行的，因为浏览器内只有一个JS引擎能够执行JS代码。
1. callback: callback实现异步编程的原理是通过定时器函数，将代码扔到浏览器的事件线程里延后执行。
2. promise: 异步编程串行化，解决异步编程执行顺序不可控的一种方式。
3. 多线程[webworker]: 浏览器提供的多线程能力。多线程属于实现异步编程的一种方式，但这不是多线程的唯一功能。
